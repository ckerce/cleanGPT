...
class FactoredCausalSelfAttentionALiBi(nn.Module):
    ...
    ARCHITECTURE DESIGN:
    - Q,K projections computed from normalized combined state (xt + xe) for contextual awareness
    - Values come from xt stream to preserve symbolic token-like structure
    - Optional V projection (use_v=True) transforms xt before attention application
    - Optional output projection (use_proj=True) applied after attention, before skip connection
    
    DATA FLOW (matches vanilla transformer structure):
    1. Q, K = norm(xt + xe) @ [W_q, W_k]  # Contextual query/key computation
    2. V = xt @ W_v if use_v else xt       # Symbolic value computation  
    3. attn_weights = softmax(Q @ K.t())   # Standard attention weights
    4. attn_output = attn_weights @ V      # Apply attention to values
    5. output = attn_output @ W_o if use_proj else attn_output  # Output projection
    6. xt_new = xt + output               # Skip connection (applied in block)
    
    This preserves both:
    - Vanilla transformer semantics: projections inside attention, skip outside
    - Factored model semantics: xt maintains symbolic structure, xe provides context
    
    FLAGS:
    - use_v=False: xt used directly as values (pure symbolic manipulation)
    - use_v=True: xt projected through learnable V matrix (enhanced symbolic transformation)
    - use_proj=False: no output projection (attention output used directly)  
    - use_proj=True: attention output projected before skip connection
    """
    ...
    def _get_kronecker_lifted_tensor(self, v):
        """
        Lifts a n_heads × n_heads matrix to n_embd × n_embd via Kronecker product structure.
        ...
        INTUITION:
        - Creates block-diagonal structure respecting multi-head architecture
        - Each head's transformation is scaled by corresponding v[i,j] element
        - Preserves head separation while allowing cross-head interactions via v matrix
        - Reduces parameters from O(n_embd²) to O(n_heads²) while maintaining expressivity
        
        USAGE:
        - V projection: transforms symbolic xt tokens before attention application
        - Output projection: transforms attention output before skip connection
        Both maintain factored model's dimensional analysis principles.
    ...
    def forward(self, x_norm_for_qk, xt_current_for_v):
        """
        Forward pass implementing factored attention with proper transformer semantics.
        
        CRITICAL DESIGN DECISIONS:
        1. Q,K computed from normalized combined state (x_norm_for_qk = norm(xt + xe))
           → Provides contextual awareness for attention pattern computation
           
        2. Values derived from xt stream (xt_current_for_v = xt)  
           → Preserves symbolic token-like structure in value computation
           
        3. V projection (if enabled) applied to xt before attention
           → Transforms symbolic tokens while preserving factored semantics
           → Creates learnable value space while maintaining xt's token-like nature
           
        4. Output projection (if enabled) applied after attention, before skip connection
           → Matches vanilla transformer: proj inside attention, skip outside
           → Allows gradient flow through transformation before residual path
        
        DATA FLOW EQUIVALENCE TO VANILLA TRANSFORMER:
        Vanilla:     x_new = x + W_o @ (attn_weights @ (x @ W_v))
        Factored:    xt_new = xt + W_o @ (attn_weights @ (xt @ W_v))
                     where attn_weights = softmax(norm(xt+xe) @ W_q @ W_k.t() @ norm(xt+xe).t())
        
        This preserves transformer semantics while enabling factored xt/xe stream processing.
        
        Args:
            x_norm_for_qk: Normalized combined state (xt + xe) for Q,K computation
            xt_current_for_v: Current xt stream for value computation
            
        Returns:
            Attention output to be added to xt via skip connection
        """
        ...
        # DESIGN CHOICE: V projection placement preserves transformer semantics
        if self.use_v:
            # Apply Kronecker-lifted V matrix to xt (symbolic token transformation)
            # This transforms the symbolic space while preserving factored structure
            ...
            # Use xt directly as values (pure symbolic manipulation, no transformation)
            # This preserves the original factored model's symbolic reasoning properties
        ...
        # DESIGN CHOICE: Output projection before skip connection (matches vanilla transformer)
        if self.use_proj:
            # Apply Kronecker-lifted output projection
            # This transforms attention output before residual connection
            # Placement matches vanilla: W_o applied inside attention, skip connection outside
...
class FactoredPreLNBlockALiBi(nn.Module):
    ...
    - xt stream: Updated by attention mechanism (symbolic token manipulation)
    - xe stream: Updated by MLP (contextual embedding enhancement)
    - Combined state (xt + xe) used for normalization and Q,K computation
    - Skip connections applied to respective streams
    
    ATTENTION PATH: xt → xt + attention(norm(xt + xe), xt)
    MLP PATH:       xe → xe + mlp(norm(xt + xe))
    
    This maintains stream separation while allowing cross-stream information flow
    through the normalization step, preserving the factored model's dimensional analysis.
    """
    def __init__(self, config):
        super().__init__()
    ...
    def forward(self, xt, xe):
        """
        Forward pass implementing factored transformer block with proper stream handling.
        
        CRITICAL ARCHITECTURAL DECISIONS:
        
        1. ATTENTION PATH (updates xt - symbolic token stream):
           - Normalization: Uses combined state norm(xt + xe) for contextual awareness
           - Q,K computation: From normalized combined state (contextual patterns)  
           - V computation: From xt stream only (preserves symbolic structure)
           - Skip connection: xt_new = xt_old + attention_output
           
        2. MLP PATH (updates xe - embedding enhancement stream):
           - Normalization: Uses combined state norm(xt + xe) for full context
           - MLP input: Normalized combined state (full information)
           - Skip connection: xe_new = xe_old + mlp_output
           
        3. STREAM SEPARATION PRINCIPLE:
           - xt maintains symbolic/token-like properties via attention updates
           - xe accumulates contextual/embedding-like properties via MLP updates
           - Streams interact through normalization but maintain separate identities
           
        This design enables:
        - Symbolic reasoning through xt stream attention updates
        - Contextual enhancement through xe stream MLP updates  
        - Cross-stream information flow via shared normalization
        - Dimensional analysis tracking of information types
        
        Args:
            xt: Token-like state stream
            xe: Embedding-like state stream
            
        Returns:
            (xt_new, xe_new): Updated streams maintaining factored structure
        """
