...
def test_length_extrapolation(model, tokenizer, device, config):
    """
    ...
    for i, prompt in enumerate(test_prompts):
        ...
        for gen_len in generation_lengths:
            if prompt_length + gen_len > config.max_position_embeddings:
                print(f"Skipping generation length {gen_len} (would exceed max_position_embeddings)")
                continue
    ...
    if not args.skip_generation:
        ...
        for i, prompt_text in enumerate(test_prompts):
            ...
            try:
                ...
                logger.error(f"Error generating text for prompt '{prompt_text}': {e}")
