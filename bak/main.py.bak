...
def main():
    ...
    # 3. Initialize Model 
    ###################################################################
    # Define which model architecture to use - can be "SASP" or "Vanilla"
    model_type = "SASP"  # Change this to use different architectures
    
    print(f"\nInitializing Model: {model_type}")
    model = get_model(model_type, config=model_config)
    ...
    # Add this code to the inference section of main.py, replacing the existing code

    # Define a list of starting prompts
    starting_prompts = [
        "The history of",
    ...
    try:
        ...
        for prompt_index, start_text in enumerate(starting_prompts):
            print(f"\n[Prompt {prompt_index + 1}]: '{start_text}'")
            ...
            generated_ids, generated_text = run_generation(
                ...
                device=config.DEVICE,
                max_new_tokens=config.GENERATION_MAX_LEN,
                temperature=0.8,  # Using slightly higher temperature for diversity
                top_k=50         # Using top-k sampling to prevent repetition
            )
